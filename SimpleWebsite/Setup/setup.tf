...
terraform {
  required_providers {
    aws = {
      version = "5.43.0" # Specifies the version of the AWS provider to use
    }
  }
}
...
resource "aws_s3_bucket" "tfstate" {
  bucket        = var.tfstate_bucket_name # Name of the S3 bucket
...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name           = var.state_lock_table # Name of the DynamoDB table
